...
{
    public class DeviceHub : Hub, IHub
    {
        public readonly DeviceDbService _deviceDbService;

        public DeviceHub(DeviceDbService deviceDbService)
        {
        ...
        public async Task SubscribeById(string docId)
        {
            ...
            var pipeline = new EmptyPipelineDefinition<ChangeStreamDocument<DeviceDto>>()
                    .Match(t => t.FullDocument.Id == Guid.Parse(""));

            Guid subscriptionId = _deviceDbService.Subscribe(pipeline, options);
        }
    }
}
